...
As tags pertencem a um processo modelado e são associadas às suas requisições. Renomear ou alterar a cor de uma tag altera sua apresentação em todas as requisições associadas, inclusive encerradas, com impacto explicitado ao usuário e registro de quem alterou o quê e quando.
...
A cor faz parte da tag no catálogo e usa o mesmo seletor das categorias de processos. As pills mantêm contraste de texto automático; a auditoria registra as cores anterior e nova tanto no catálogo quanto nas execuções associadas. A autoria original da associação permanece preservada.
