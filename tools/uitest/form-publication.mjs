...
const field=(key,row,columns)=>({id:key,key,label:key,kind:'field',type:'textfield',config:{layout:{row,columns}}});
const schema=(name)=>({format:'septem-native',schemaVersion:1,id:'form-publication',tabs:[
 {id:'tab-main',label:name,groups:[{id:'principal',type:'group',label:'Principal',fields:[field('Primeiro','r1',8),field('Segundo','r1',8),field('Terceiro','r2',8),field('Quarto','r3',8),field('Quinto','r3',8)]}]},
 {id:'tab-other',label:'Outro grupo',groups:[{id:'outro',type:'group',label:'Outro',fields:[field('Outro_campo','r4')]}]}
]});
try{
 for(const width of [1280,375]){
  ...
  const boxes=await page.locator('input:visible').evaluateAll(inputs=>inputs.map(x=>{const r=x.closest('.septem-form-grid > div').getBoundingClientRect();return {x:r.x,y:r.y,w:r.width};}));
  if(width>640){assert.equal(boxes[0].y,boxes[1].y);assert.ok(boxes[2].y>boxes[1].y);assert.ok(boxes[1].x>boxes[0].x);assert.equal(boxes[2].y,boxes[3].y);assert.ok(boxes[4].y>boxes[3].y);assert.equal(boxes[3].w,boxes[4].w);}
  ...
  await page.getByRole('tab',{name:'Outra sessão, 0 pendências',exact:true}).waitFor();
