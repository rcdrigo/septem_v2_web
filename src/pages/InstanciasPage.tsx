...
import { TagPills, TagsButton, TagHistoryButton } from '@/components/tags';
...
function RequestCards({ items, onOpen }: { items: InstanceListItem[]; onOpen: (id: string) => void }) { return <div className="grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => <article key={item.id} data-testid="req-card" role="link" tabIndex={0} onClick={() => onOpen(item.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onOpen(item.id); } }} className="group min-w-0 cursor-pointer rounded-lg border border-slate-200 bg-white p-4 shadow-sm hover:border-slate-300 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-700"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><RequestProcess item={item} /></div><span className="shrink-0 text-xs font-semibold tabular-nums text-slate-500">{item.number != null ? `#${item.number}` : 'Sem número'}</span></div><p title={item.inboxText || undefined} className="mt-3 line-clamp-2 min-h-10 text-sm leading-5 text-slate-500">{item.inboxText || 'Sem resumo disponível.'}</p><TagPills tags={item.tags ?? []} className="mt-3" /><div data-testid="req-card-footer" className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-slate-100 pt-3 text-xs"><StatusBadge status={item.status} /><span className="inline-flex items-center gap-1 text-slate-600" title="Início"><CalendarClock size={13} className="shrink-0 text-slate-400" />{fmt(item.startedAt)}</span><span className="inline-flex items-center gap-1 text-slate-600" title="Conclusão"><CalendarCheck2 size={13} className="shrink-0 text-slate-400" />{item.endedAt ? fmt(item.endedAt) : '—'}</span></div><span className="task-access mt-3 flex items-center justify-end gap-1 text-xs font-semibold text-slate-700 md:opacity-0 md:group-hover:opacity-100 md:group-focus-within:opacity-100">Acessar <ArrowRight size={14} /></span></article>)}</div>; }
function RequestTable({ items, onOpen }: { items: InstanceListItem[]; onOpen: (id: string) => void }) { return <div className="rounded-lg border border-slate-200 bg-white"><table className="w-full table-fixed text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="w-[28%] px-4 py-3 text-left">Processo</th><th className="w-[32%] px-4 py-3 text-left">Resumo</th><th className="w-[15%] px-4 py-3 text-left">Status</th><th className="w-[17%] px-4 py-3 text-left">Início / fim</th><th className="w-[8%] px-4 py-3" /></tr></thead><tbody>{items.map((item) => <tr key={item.id} tabIndex={0} onClick={() => onOpen(item.id)} onKeyDown={(event) => { if (event.key === 'Enter') onOpen(item.id); }} className="group cursor-pointer border-t border-slate-100 hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-slate-700"><td className="px-4 py-3"><RequestProcess item={item} /><p className="mt-1 truncate text-xs text-slate-500">{item.number != null ? `#${item.number}` : 'Sem número'}</p></td><td className="min-w-0 px-4 py-3 text-slate-500"><p title={item.inboxText || undefined} className="truncate">{item.inboxText || '—'}</p><TagPills tags={item.tags ?? []} className="mt-2" /></td><td className="px-4 py-3"><StatusBadge status={item.status} /></td><td className="px-4 py-3 text-xs text-slate-500">{fmt(item.startedAt)}<br />{item.endedAt ? fmt(item.endedAt) : '—'}</td><td className="px-4 py-3"><span className="task-access inline-flex items-center gap-1 whitespace-nowrap text-xs font-semibold opacity-0 group-hover:opacity-100 group-focus:opacity-100">Acessar <ArrowRight size={14} /></span></td></tr>)}</tbody></table></div>; }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        ...
        <TagHistoryButton executionId={id} />
